...
int Post_Order(tree T,long int * min,long long int* sum)
{
    if(T== NULL)
        return 0;
    else
    {
        int left = Post_Order(T->left,min,sum);
        int right = Post_Order(T->right,min,sum);
        int current = T->data;
        if(left ==0 && right == 0)
            return current;
        if(isLite(left,current,right))
        {
            // printf("%d %d %d--is a lite tree",left,current,right);
            if((*min)>(left + right + current))
                *min = left + right + current;
        }
        else if (!isLite(left,current,right))
        {
            // printf("%d %d %d--is not a lite tree",left,current,right);
            if((*min)>(left + right + current))
                *min = left + right + current;
        }
        return current;
